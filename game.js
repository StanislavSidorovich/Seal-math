...
// Russian plural forms for "вопрос" (1 вопрос, 2 вопроса, 5 вопросов, 11 вопросов...)
function ruPluralQuestions(n) {
...
function launchGame() {
  ...
  randomSurprise();
  if (state.activeTrip) {
    // App process was killed (or the tab closed) mid-mission — resume it
    // instead of dropping the player back at the map (P16, see startMission()
    // / answer() / completeMission() for where this snapshot is maintained).
    startMission(state.activeTrip.daily, state.activeTrip);
...
// No-op outside the native Android wrapper. Registering this listener hands
// Capacitor's default back-button behavior (exit the app from any screen)
// over to us entirely, so every branch below has to explicitly decide what
// "back" means instead of the OS just closing the app out from under a mid
// -mission child. Each closer here is the exact function the matching
...
function startMission(daily, resumeTrip = null) {
  if (miniGameTimer) { clearTimeout(miniGameTimer); miniGameTimer = null; }
  switchView("adventure");
  if (resumeTrip) {
    // Resuming after the app process was killed mid-mission (see launchGame())
    // — reuse the exact in-progress trip instead of starting over at Q1.
    trip = resumeTrip;
    selectedWorld = trip.world;
  } else {
  ...
  if (cloudsEl) cloudsEl.hidden = trip.mission !== 4;
  if (trip.mission === 4) syncStormVisuals();
...
function makeProblem() {
  ...
  const labels  = currentProblem.optionLabels; // e.g. fracCompare: show the fraction, not its numeric code
  $("answers").innerHTML = choices.map((v,i) =>
    `<button class="answer" data-value="${v}" aria-label="Answer: ${labels ? labels[v] : v}" tabindex="${i===0?0:-1}">${labels ? labels[v] : v}</button>`
  ).join("");
  document.querySelectorAll(".answer").forEach(btn =>
    btn.addEventListener("click", () => answer(Number(btn.dataset.value), btn))
  );
...
const twoStepTemplates = [
  (a,b,c,ch,isRu) => isRu
    ? { text:`Колбаска поймал ${a} рыбок. Он отдал ${b} из них друзьям. Потом поймал ещё ${c}. Сколько рыбок у него теперь?`, answer:a-b+c, hint:`Сначала отними отданных рыбок, потом прибавь новых.` }
    : { text:`Sausage caught ${a} fish. He gave ${b} to ${ch}. Then he caught ${c} more. How many fish does he have now?`,   answer:a-b+c, hint:`First subtract the fish given away, then add the new ones.` },
  (a,b,c,ch,isRu) => isRu
    ? { text:`У ${ruCh(ch,"gen")} было ${a} снежков. ${b} растаяли на солнце. Потом добавилось ещё ${c} новых. Сколько снежков теперь?`, answer:a-b+c, hint:`Отними растаявшие, потом прибавь новые.` }
    : { text:`${ch} had ${a} snowballs. ${b} melted in the sun. Then she made ${c} new ones. How many snowballs now?`,        answer:a-b+c, hint:`Subtract the melted ones, then add the new ones.` },
  (a,b,c,ch,isRu) => isRu
    ? { text:`На льду было ${a} пингвинов. ${b} из них нырнули в воду. Потом приплыло ещё ${c}. Сколько пингвинов сейчас?`, answer:a-b+c, hint:`Отними тех, кто уплыл, потом прибавь новых.` }
    : { text:`There were ${a} penguins on the ice. ${b} dived away. Then ${c} more arrived. How many penguins are there?`,    answer:a-b+c, hint:`Take away those that left, then add the arrivals.` },
  (a,b,c,ch,isRu) => isRu
    ? { text:`Колбаска нашёл ${a} золотых монет. Он потратил ${b} на рынке. Потом получил ещё ${c} в подарок. Сколько монет у него теперь?`, answer:a-b+c, hint:`Отними потраченное, потом прибавь подарок.` }
    : { text:`Sausage found ${a} treasure coins. He spent ${b} at the market. ${ch} gave him ${c} more. How many coins?`,    answer:a-b+c, hint:`Subtract what was spent, then add the gift.` },
  (a,b,c,ch,isRu) => isRu
    ? { text:`В воде плавало ${a} тюленей. ${b} из них выбрались отдохнуть на лёд. Потом ${c} вернулись обратно. Сколько тюленей плавает сейчас?`, answer:a-b+c, hint:`Отними тех, кто отдыхал, прибавь тех, кто вернулся.` }
    : { text:`${a} seals were swimming. ${b} went to rest on the ice. Then ${c} came back. How many are swimming now?`,       answer:a-b+c, hint:`Take away those that rested, add the ones that returned.` },
  (a,b,c,ch,isRu) => isRu
    ? { text:`На лодке было ${a} ящиков. Она сгрузила ${b} у пристани и взяла ${c} новых. Сколько ящиков на лодке теперь?`, answer:a-b+c, hint:`Отними сгруженные ящики, прибавь новые.` }
    : { text:`The boat had ${a} crates. It dropped off ${b} at the dock and picked up ${c} new ones. How many crates now?`,  answer:a-b+c, hint:`Subtract the crates dropped off, add the new ones.` },
  // multiply then add
  (a,b,c,ch,isRu) => isRu
    ? { text:`У ${ruCh(ch,"gen")} есть ${a} корзин по ${b} рыбок в каждой. На берегу нашлось ещё ${c} рыбок. Сколько рыбок всего?`, answer:a*b+c, hint:`Умножь, чтобы найти рыбок в корзинах, потом прибавь лишних.` }
    : { text:`${ch} has ${a} baskets with ${b} fish each. She finds ${c} more fish on the shore. How many fish in total?`,   answer:a*b+c, hint:`Multiply to find fish in baskets, then add the extra ones.` },
  (a,b,c,ch,isRu) => isRu
    ? { text:`Есть ${a} лодок, и в каждой по ${b} пингвинов. Ещё ${c} пингвинов запрыгнули на борт. Сколько пингвинов всего?`, answer:a*b+c, hint:`Сначала умножь, потом прибавь лишних пингвинов.` }
    : { text:`There are ${a} boats with ${b} penguins each. ${c} more penguins jump aboard. How many penguins total?`,       answer:a*b+c, hint:`Multiply first, then add the extra penguins.` },
  // add then multiply
  (a,b,c,ch,isRu) => isRu
    ? { text:`У Колбаски и ${ruCh(ch,"gen")} вместе ${a} + ${b} рыбок. Они хотят утроить это количество. Сколько получится?`, answer:(a+b)*c, hint:`Сначала сложи, потом умножь на ${c}.` }
    : { text:`Sausage and ${ch} together have ${a} + ${b} fish. They want to triple that amount. What is the total?`,        answer:(a+b)*c, hint:`Add first, then multiply by ${c}.` },
];
...
function generateTwoStep() {
  ...
  let prob;
  if (idx === 6 || idx === 7) {
    prob = tpl(mulA, mulB, mulC, char, isRu);
  } else if (idx === 8) {
    const addA=rand(15)+3, addB=rand(12)+2, mult=rand(3)+2;
    prob = tpl(addA, addB, mult, char, isRu);
  } else {
    prob = tpl(a, b, c, char, isRu);
  }
...
function generateAdvEquation() {
  ...
  const forms = [
    ...
    () => { const x=rand(30)+5, a=rand(20)+10; return { text:`x + ${a} = ${x+a}`, answer:x, hint: isRu ? "Отними, чтобы найти x." : "Subtract to find x." }; },
    // large - x = medium (e.g. 56 - x = 19)
    () => { const x=rand(25)+5, total=rand(30)+x+5; return { text:`${total} - x = ${total-x}`, answer:x, hint: isRu ? "Отними ответ от большого числа, чтобы найти x." : "Subtract the answer from the big number to find x." }; },
    // 3 × x = result
    () => { const f=rand(4)+2, x=rand(10)+2; return { text:`${f} × x = ${f*x}`, answer:x, hint: isRu ? `Раздели ${f*x} на ${f}, чтобы найти x.` : `Divide ${f*x} by ${f} to find x.` }; },
    // x ÷ f = result
    () => { const f=rand(5)+2, x=rand(8)+2; return { text:`x ÷ ${f} = ${x}`, answer:x*f, hint: isRu ? `Умножь ${x} на ${f}, чтобы найти x.` : `Multiply ${x} by ${f} to find x.` }; },
    // 2x = result  (written as 2 × x)
    () => { const x=rand(15)+3; return { text:`2 × x = ${2*x}`, answer:x, hint: isRu ? `Раздели ${2*x} на 2, чтобы найти x.` : `Divide ${2*x} by 2 to find x.` }; },
    // x² style: x × x = ? (only level 10+)
    () => { const x=rand(6)+2; return { text:`x × x = ${x*x}`, answer:x, hint: isRu ? `Какое число, умноженное само на себя, даёт ${x*x}?` : `What number multiplied by itself gives ${x*x}?` }; },
  ];
...
function generateReverseMul() {
  ...
  const isRu = currentLang === "ru";
  const think = isRu ? "Подумай" : "Think";
...
function generateFracCompare() {
  ...
  const pairs = [
    { a:[1,2], b:[1,4], answer:1, hint:"½ is bigger than ¼ — imagine cutting a pizza.", hintRu:"½ больше, чем ¼ — представь, что режешь пиццу." },
    { a:[1,4], b:[1,2], answer:2, hint:"½ is bigger than ¼.", hintRu:"½ больше, чем ¼." },
    { a:[3,4], b:[1,2], answer:1, hint:"¾ is bigger than ½.", hintRu:"¾ больше, чем ½." },
    { a:[1,3], b:[1,4], answer:1, hint:"The smaller the bottom number, the bigger the slice.", hintRu:"Чем меньше число снизу, тем больше кусок." },
    { a:[2,4], b:[1,2], answer:0, hint:"2/4 and 1/2 are the same — they are equivalent fractions.", hintRu:"2/4 и 1/2 — это одно и то же, равные дроби." },
    { a:[1,5], b:[1,3], answer:2, hint:"1/3 is bigger — thirds are larger slices than fifths.", hintRu:"1/3 больше — трети крупнее, чем пятые." },
    { a:[3,5], b:[1,2], answer:1, hint:"3/5 = 0.6 and 1/2 = 0.5, so 3/5 is bigger.", hintRu:"3/5 = 0,6, а 1/2 = 0,5, значит 3/5 больше." },
    { a:[2,3], b:[3,4], answer:2, hint:"¾ = 0.75, ⅔ ≈ 0.67, so ¾ is bigger.", hintRu:"¾ = 0,75, а ⅔ ≈ 0,67, значит ¾ больше." },
  ];
  ...
  const aStr = fracStr(an,ad), bStr = fracStr(bn,bd);
  const isRu = currentLang === "ru";

  return {
    topic:"fracCompare",
    text: isRu
      ? `Какая дробь больше? (1) ${aStr}   или   (2) ${bStr}   — введи 1, 2, или 0, если поровну`
      : `Which is larger? (1) ${aStr}   or   (2) ${bStr}   — type 1, 2, or 0 if equal`,
    answer:p.answer,
    hint: isRu ? p.hintRu : p.hint,
    optionLabels: isRu ? {0:"Поровну",1:aStr,2:bStr} : {0:"Equal",1:aStr,2:bStr}
  };
}
...
function wrongAnswers(problem) {
  ...
  // fracCompare only has three valid codes (0,1,2) — the two non-correct
  // codes above are the complete distractor set, so skip the numeric
  // fallback below (it would otherwise pad in a meaningless 4th number).
  let attempts = 0;
  while (problem.topic !== "fracCompare" && values.size < 3 && attempts < 30) {
    attempts++;
    // when correct < spread, bias the offset range upward so clamping
    // negative offsets to 0 can't eat the full distractor count (P1-2)
    const bias = correct < spread ? spread - correct : 0;
    const offset = Math.floor(Math.random() * (spread*2+1)) - spread + bias || (Math.random()<.5?-1:1);
...
function answer(value, btn) {
  ...
  document.querySelectorAll(".answer").forEach(b => {
    ...
    if (Number(b.dataset.value) === currentProblem.answer) b.classList.add("correct");
  });
  ...
  if (correct) {
    ...
    if (trip.mission === 4) {
      ...
      $("missionTrailMarker").style.left = `${missionTrailPos(trip.solved, trip.needed)}%`;
      syncMissionSeal();
      setTimeout(() => trip.solved >= trip.needed ? completeMission() : makeProblem(), 1000);
    }
  } else {
    state.wrong++;
  ...
  checkAchievements();
  // Keep the persisted snapshot current so a killed/reopened app resumes this
  // mission's progress instead of restarting it at question 1 (see
  // launchGame()'s state.activeTrip check and completeMission()'s clear of it).
  state.activeTrip = trip.active ? { ...trip } : null;
  save();
...
function completeMission() {
  ...
  // state.activeTrip, which would wrongly resume it on the next app launch.
  trip.active = false;
  state.activeTrip = null;
...
function initAudio() {
  ...
  audioReady = true;
  // Some Android WebViews create the context already suspended even inside
  // a user-gesture handler like this one, and every scheduled tone/oscillator
  // silently no-ops on a suspended context instead of throwing — resume()
  // is a no-op if the context is already running, so this is safe everywhere.
  if (audioCtx.state === "suspended") audioCtx.resume();
  // The OS can also suspend the context when the app is backgrounded; without
  // this, sound stays dead after the child returns from a call/notification.
